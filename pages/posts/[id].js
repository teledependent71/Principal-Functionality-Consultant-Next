...
import postsPageInitialPathsE8193Resource from '../../resources/posts-page-initial-paths-e8193'
import postsPageInitialProps4f90dResource from '../../resources/posts-page-initial-props-4f90d'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Principal Functionality Consultant</title>
          <meta
            property="og:title"
            content="Posts1 - Principal Functionality Consultant"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsE8193Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps4f90dResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
